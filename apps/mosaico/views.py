...
def download(request):
    ...
    if action == "download":
        ...
        msg = ""
        send_mail(
            subject, msg, from_email, [to], html_message=html, fail_silently=False
        )
...
def template(request):
    ...
    if action == "save":
        ...
        template, created = Template.objects.get_or_create(key=key, name=name)
        template.html = html
...
def get_placeholder_image(width, height):
    ...
    while y < height:
        ...
        draw.polygon(
            [(x, y + size), (x + size, y + size * 2), (x, y + size * 2)],
            fill=(0x80, 0x80, 0x80),
        )
        x = x + size * 2
        if x > width:
